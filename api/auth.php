...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'login') {
        ...
        if ($foundUser) {
			...
			$sessionRoles = normalizeUserRoles($foundUser);
			$_SESSION['logged_in'] = true;
			$_SESSION['user_id'] = $foundUser['id'];
            $_SESSION['name'] = $foundUser['name'];
            $_SESSION['login'] = $foundUser['login'];
			$_SESSION['roles'] = $sessionRoles;
			$_SESSION['role'] = primaryRoleFromList($sessionRoles);
            sendJSON([
                'success' => true,
                'user' => [
...
elseif ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($action === 'status') {
        if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true) {
            ...
            if (empty($sessionRoles) && !empty($_SESSION['role'])) {
            ...
            sendJSON([
                ...
                'user' => [
                    ...
                    'name'  => $_SESSION['name'],
                    'role'  => isset($_SESSION['role']) ? $_SESSION['role'] : primaryRoleFromList($sessionRoles),
                    'roles' => $sessionRoles
                ]
            ]);
        } else {
