...
// =============================================================================
// �sT️  CONFIGURATION SMTP �?" Chargée depuis le fichier .env (api/config.php)
...
define('SMTP_NAME',     envVar('SMTP_NAME', 'Inventaire Pompier'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if (!$input || !isset($input['vehicle_id']) || !isset($input['location_name'])
            ...
            recordFailedAttempt('alert_create');
            ...
            if ($responsibleLogin) {
                ...
                if (file_exists($usersFile)) {
                    ...
                    if ($responsibleUser && !empty($responsibleUser['email'])) {
                        $subject = "[Inventaire Pompier] Alerte : " . $newAlert['alert_type']
                                 . " sur " . $vehicleName;
        ...
        if (file_exists($vehicleFile)) {
            ...
            if ($responsibleLogin) {
                ...
                if (file_exists($usersFile)) {
                    ...
                    if ($responsibleUser && !empty($responsibleUser['email'])) {
                        $subject = "[Inventaire Pompier] Véhicule indisponible : " . $vehicleName;
    ...
    // �? partir d'ici, les actions requièrent d'être admin connecté
    requireAuth();
