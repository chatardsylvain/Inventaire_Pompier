...
/**
 * api/upload.php
 * Script d'upload d'images de véhicules et d'images partagées.
 ...
 *
 * Modifié le 2026-09-02 — Ajout du mode "shared"
 *   POST param shared=1 → nom de fichier basé sur le hash MD5 du contenu (sans préfixe véhicule)
 *   POST param shared_name=xxx → nom libre (optionnel, utilisé si fourni à la place du hash MD5)
 *   L'image partagée est stockée dans /images/ comme les autres, mais identifiable dans list_images
 *   car son nom ne suit pas le pattern vehicleId_timestamp.ext
 */
...
$isShared   = isset($_POST['shared'])       && $_POST['shared'] === '1';
$sharedName = isset($_POST['shared_name'])  ? trim($_POST['shared_name'])  : '';
...
// Mode partagé (shared=1) : nom basé sur le hash MD5 du fichier source (ou shared_name si fourni)
//   → pas de préfixe véhicule, réutilisable entre véhicules/lots
// Mode normal : vehicleId_timestamp.webp (comportement original)
if ($isShared) {
    if ($sharedName !== '') {
        // Nettoyage du nom libre fourni (lettres, chiffres, tirets, underscores)
        $cleanSharedName = preg_replace('/[^a-zA-Z0-9\-_]/', '_', $sharedName);
        $cleanSharedName = trim($cleanSharedName, '_');
        $fileName = ($cleanSharedName !== '' ? $cleanSharedName : md5_file($file['tmp_name'])) . '.webp';
    } else {
        $fileName = md5_file($file['tmp_name']) . '.webp';
    }
} else {
...
if (!$conversionSuccess) {
    if ($isShared) {
        $baseName = ($sharedName !== '')
            ? preg_replace('/[^a-zA-Z0-9\-_]/', '_', $sharedName)
            : md5_file($file['tmp_name']);
        $fileName = trim($baseName, '_') . '.' . $extension;
    } else {
        $cleanVehicleId = preg_replace('/[^a-zA-Z0-9\-]/', '_', $vehicleId);
